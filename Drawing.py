...
class Drawing:
    ...
    def to_nifty_add_layer_import(self):
        ...
        local_storage_data = """var decompressed_data = JSON.parse(LZString.decompress(JSON.parse(window.localStorage.getItem("drawing"))));"""

        # Set up the json string of the new data
        json_object = "var json_object = JSON.parse(\"" + json.dumps(self.object).replace("\"", "\\\"").replace(" ", "") + "\");"

        # Concat the new data to the end of the old saved data
        new_drawing_data = """decompressed_data['lines'] = decompressed_data['lines'].concat(json_object['lines']);"""

        # Turn the JSON object into a string
        json_string = "var json_string = JSON.stringify(decompressed_data);"
        ...
        return lz_string + local_storage_data + json_object + new_drawing_data + json_string + local_storage + refresh_page
