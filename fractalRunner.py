...
def fractalRunner(drawing):
    ...
    fd1 = fs.defns[1]
    fd1.add_child(FractalPiece2D(1, Vector2D(), Matrix2D()))
    fd1.iterate = False
    fp1 = fd1.plotter
    ...
    scv = 0.5
    scm = 0.45
    fd2.add_child(FractalPiece2D(3, Vector2D(-1, -1) * scv, Matrix2D() * scm))
    fd2.add_child(FractalPiece2D(4, Vector2D(1, -1) * scv, Matrix2D() * scm))
    fd2.add_child(FractalPiece2D(5, Vector2D(-1, 1) * scv, Matrix2D() * scm))
    fd2.add_child(FractalPiece2D(6, Vector2D(1, 1) * scv, Matrix2D() * scm))
    fd2.add_child(FractalPiece2D(7, Vector2D(-0.5, -0.8) * scv, Matrix2D.rotd(30) * 0.5 * scm))

    # Definition #3 - demo Sierpinski Sieve
    fd3 = fs.defns[3]
    sc = 0.5
    fd3.add_child(FractalPiece2D(3, Vector2D(-1, 1) * sc, Matrix2D() * sc))
    fd3.add_child(FractalPiece2D(3, Vector2D(-1, -1) * sc, Matrix2D() * sc))
    fd3.add_child(FractalPiece2D(3, Vector2D(1, -1) * sc, Matrix2D() * sc))
    ...
    fd4 = fs.defns[4]
    sc = 0.5
    fd4.add_child(FractalPiece2D(4, Vector2D(-1, -1) * sc, Matrix2D() * sc))
    fd4.add_child(FractalPiece2D(4, Vector2D(1, -1) * sc, Matrix2D() * sc))
    fd4.add_child(FractalPiece2D(4, Vector2D(-1, 1) * sc, rand_mx_square(sc ** 1.3)))
    fd4.add_child(FractalPiece2D(4, Vector2D(1, 1) * sc, rand_mx_square(sc ** 1.7)))
    # Set up plotter
    fp4 = fd4.plotter
    ...
    h = 3 ** 0.5
    id_list = [1, 5, 5, 5, 5]
    fd5.add_child(FractalPiece2D(rand_id(id_list), Vector2D(2, 0) * sc, rand_mx_circle(sc)))
    fd5.add_child(FractalPiece2D(rand_id(id_list), Vector2D(1, h) * sc, rand_mx_circle(sc)))
    fd5.add_child(FractalPiece2D(rand_id(id_list), Vector2D(-1, h) * sc, rand_mx_circle(sc)))
    fd5.add_child(FractalPiece2D(rand_id(id_list), Vector2D(-2, 0) * sc, rand_mx_circle(sc)))
    fd5.add_child(FractalPiece2D(rand_id(id_list), Vector2D(-1, -h) * sc, rand_mx_circle(sc)))
    fd5.add_child(FractalPiece2D(rand_id(id_list), Vector2D(1, -h) * sc, rand_mx_circle(sc)))
    fd5.add_child(FractalPiece2D(rand_id(id_list), Vector2D(0, 0) * sc, rand_mx_circle(sc)))
    # Set up plotter
    fp5 = fd5.plotter
    ...
    fd6 = fs.defns[6]
    sc = 0.5
    fd6.add_child(FractalPiece2D(6, Vector2D(-1, -1) * sc, Matrix2D() * sc))
    fd6.add_child(FractalPiece2D(6, Vector2D(1, -1) * sc, Matrix2D() * sc))
    fd6.add_child(FractalPiece2D(6, rand_vect_cts(-sc, sc, sc, sc), Matrix2D() * sc))
    # Set up plotter
    fp6 = fd6.plotter
    ...
    fd7 = fs.defns[7]
    sc = 0.5 ** 0.5
    fd7.add_child(FractalPiece2D(7, Vector2D(-0.5, 0.5), Matrix2D.rotd(-45) * sc))
    fd7.add_child(FractalPiece2D(7, Vector2D(0.5, 0.5), Matrix2D.rotd(-135) * sc))
    # Set up plotter
    fp7 = fd7.plotter
