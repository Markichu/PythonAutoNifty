...
class Drawing:
    ...
    def export_raw_data(self, file_name, indent=4):
        with open(file_name, "w") as file:
            json.dump(self.to_nifty_structured_object(), file, indent=indent)
    ...
    def import_raw_data(self, file_name):
        ...
        self.object = self.from_nifty_structured_object(drawing_data)
    ...
    def to_nifty_structured_object(self):
        temp = []
        for line in self.object['lines']:
            temp.append({
    ...
    def from_nifty_structured_object(self, json_dict):
        temp = []
    ...
    def to_nifty_show_import(self):
        return "drawingCanvas.current.loadSaveData(\"" + json.dumps(self.to_nifty_structured_object()).replace('"', '\\"') + "\", false)"
    ...
    def to_nifty_import(self):
        ...
        json_string = "var json_string = \"" + json.dumps(self.to_nifty_structured_object()).replace("\"", "\\\"").replace(" ", "") + "\";"
    ...
    def to_nifty_add_layer_import(self):
        ...
        # Raw json string
        raw_json_string = json.dumps(self.to_nifty_structured_object()).replace("\"", "\\\"").replace(" ", "")
